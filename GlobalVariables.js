...
$("#minus").click(() => {
  ...
  const paraSize = $(":root").css(`${var2}`).split("r");
  // console.log(iconSize);
  const paraSizeFloat = Math.max(0.7, parseFloat(paraSize[0]));

  if (paraSizeFloat > 0.7)
    $(":root").css(`${var2}`, paraSizeFloat - 0.1 + "rem");
...
function editMenu() {
  ...
  if (
    divMenuContainer.getBoundingClientRect().height <
    ulMenu.getBoundingClientRect().height
  )
    divMenuContainer.style.height =
      ulMenu.getBoundingClientRect().height + 10 + "px";
}
